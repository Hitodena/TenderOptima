...
function RequestCard({ request, tab, onComplete, onActivate }: RequestCardProps) {
  ...
  // Load response count for this request using correct API endpoint
  const { data: responseCount } = useQuery<number>({
    queryKey: [`/api/supplier-responses-by-request`, request.id],
    queryFn: async () => {
      try {
        const responses = await apiRequest<any[]>(`/api/supplier-responses-by-request/${request.id}`, 'GET');
    ...
    >
      <div className="flex items-center justify-between p-3 h-14">
        ...
        <div className="flex items-center gap-6">
          {/* Кружок с количеством ответов - показываем только если есть ответы */}
          {(responseCount && responseCount > 0) && (
            <Tooltip>
              <TooltipTrigger asChild>
                <div className="flex items-center justify-center w-8 h-8 border-2 border-gray-300 rounded-full bg-transparent cursor-help">
