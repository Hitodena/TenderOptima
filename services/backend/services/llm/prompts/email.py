...
если цены нет — null
   ...
   - Для цены за единицу ищи значения в таблицах КП, строках «цена», «за шт», «за ед.», \
«unit price», «стоимость единицы», колонках с количеством и суммой
   - Если в письме указаны только общая сумма и количество — вычисли numeric_value как \
сумма / количество (округление до 4 знаков) и отрази расчёт в offer_value
   - Не пропускай цену за единицу, если она явно есть в таблице или тексте, даже при \
наличии только общей суммы по позиции
