...
export const translations = {
	...
	admin: {
		...
		analysisDebug: {
			...
			totalLabel: 'Анализов:',
			description:
				'Вложения писем и извлечённые параметры с пометками происхождения — для доработки промптов.',
			searchPlaceholder: 'Поиск: тема, email, заявка, поставщик',
			withAttachmentsOnly: 'Только с вложениями',
			empty: 'Анализов нет',
			loadError: 'Не удалось загрузить анализы',
			detailError: 'Не удалось загрузить детали анализа',
			selectHint: 'Выберите письмо слева',
			noSubject: 'Без темы',
			attachmentsShort: 'Файлы',
			matchesShort: 'Параметры',
			attachmentsTitle: 'Вложения / скрины',
			matchesTitle: 'Извлечённые параметры',
			noAttachments: 'Вложений нет',
			noMatches: 'Параметров нет',
			download: 'Скачать',
			...
			originExtracted: 'извлечено с AI',
			originCalculated: 'рассчитано',
			originManual: 'вручную',
			originEmpty: 'нет значения',
		},
		users: {
	...
	cookies: {
		...
		necessaryDescription:
			'Данные cookie-файлы требуются для обеспечения функционирования сайта, в том числе корректного использования предоставляемых возможностей и услуг, и не подлежат отключению.',
		analyticsTitle: 'Аналитические cookie-файлы',
		analyticsDescription:
			'Данные cookie-файлы необходимы в статистических целях, позволяют подсчитывать количество и длительность посещений сайта и анализировать, как посетители используют сайт.',
	...
	subscription: {
		...
		allLimits: 'Все лимиты',
		searches: 'Поиски',
		emails: 'Email',
		kpProcessed: 'КП обработано',
		pagesAnalyzed: 'Страниц проанализировано',
		uploadLimit: 'Лимит вложения',
		module1Usage: 'Модуль 1 — использование',
		module2Usage: 'Модуль 2 — использование',
		activeUntil: 'Активна до {date}',
		activeFromUntil: 'Активна с {start} до {end}',
		activeFromNoEnd: 'С {date}, без даты окончания',
		expiresIn: 'Истекает через {days}',
		noExpiry: 'Без ограничения по сроку',
		...
		selectPlan: 'Выбрать',
		selectedPlan: 'Выбран',
		changePlanDisclaimer:
			'Клиент оплачивает полную цену нового тарифа. Неизрасходованный остаток лимитов текущего месяца переносится; возврат за старый тариф не делается.',
		confirmChangePlan: 'Подтвердить смену тарифа',
		invoiceAfterSwitch:
			'После подтверждения «Выставить счёт» выставит счёт уже на выбранный тариф.',
		changePlanSuccess: 'Тариф обновлён',
	},
	requests: {
