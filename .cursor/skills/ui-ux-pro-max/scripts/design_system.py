...
class DesignSystemGenerator:
    ...
    def _multi_domain_search(
        self, query: str, style_priority: list = None
    ) -> dict:
    ...
    def _select_best_match(
        self, results: list, priority_keywords: list
    ) -> dict:
        ...
        for priority in priority_keywords:
            ...
            for result in results:
                ...
                if (
                    priority_lower in style_name
                    or style_name in priority_lower
                ):
                    return result
    ...
    def generate(self, query: str, project_name: str = None) -> dict:
        ...
        typography_results = self._extract_results(
            search_results.get("typography", {})
        )
        landing_results = self._extract_results(
            search_results.get("landing", {})
        )
        ...
        return {
            ...
            "pattern": {
                "name": best_landing.get(
                    "Pattern Name",
                    reasoning.get("pattern", "Hero + Features + CTA"),
                ),
                "sections": best_landing.get(
            ...
            "typography": {
                ...
                "google_fonts_url": best_typography.get(
                    "Google Fonts URL", ""
                ),
...
def format_ascii_box(design_system: dict) -> str:
    ...
    if pattern.get("conversion"):
        lines.append(
            f"|     Conversion: {pattern.get('conversion', '')}".ljust(
                BOX_WIDTH
            )
            + "|"
        )
...
def format_master_md(design_system: dict) -> str:
    ...
    if pattern.get("conversion"):
        lines.append(
            f"- **Conversion Strategy:** {pattern.get('conversion', '')}"
        )
    if pattern.get("cta_placement"):
        lines.append(
            f"- **CTA Placement:** {pattern.get('cta_placement', '')}"
...
def _generate_intelligent_overrides(
    ...
    if style_results:
        ...
        if any(
            kw in keywords.lower()
            for kw in ["data", "dense", "dashboard", "grid"]
        ):
            layout["Max Width"] = "1400px or full-width"
            layout["Grid"] = "12-column grid for data flexibility"
            spacing["Content Density"] = (
                "High — optimize for information display"
            )
        elif any(
            kw in keywords.lower()
            for kw in ["minimal", "simple", "clean", "single"]
        ):
