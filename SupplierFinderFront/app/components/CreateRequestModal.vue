		...
		<template #body>
			<UForm :schema="schema" :state="form" @submit="handleSubmit" class="space-y-4">
				...
				<UFormField label="Запрос" name="query" required hint="Что ищем? Например: промышленные насосы">
					<UInput v-model="form.query" placeholder="Промышленные насосы" icon="i-lucide-search"
						class="w-full" />
				</UFormField>
				...
				<div class="grid grid-cols-2 gap-3">
					...
					<UFormField label="Количество" name="quantity">
						<UInput v-model="form.quantity" type="number" min=0 placeholder="100" icon="i-lucide-hash"
							class="w-full" />
					</UFormField>
				</div>
				...
				<div class="grid grid-cols-3 gap-3">
					...
					<UFormField label="Макс. цена за ед." name="max_price_per_unit">
						<UInput v-model="form.max_price_per_unit" type="number" min=0 placeholder="1000"
							class="w-full" />
					</UFormField>
...
const schema = z.object({
	...
	quantity: z.coerce.number().gt(0, "Количество не может быть меньше или равно 0").optional(),
	unit: z.string().max(50).optional(),
	quality_requirements: z.string().max(1000).optional(),
	delivery_deadline: z.string().optional(),
	max_price_per_unit: z.coerce.number().gt(0, "Цена не может быть меньше или равна 0").optional(),
...
async function handleSubmit() {
	...
	try {
		const payload: RequestCreate = {
			...
			max_price_per_unit: form.max_price_per_unit ? Number(form.max_price_per_unit) : null,
