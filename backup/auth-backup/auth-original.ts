...
export async function setupAuth(app: Express) {
  ...
  const sessionSettings: session.SessionOptions = {
    secret: process.env.SESSION_SECRET || 'supplier-finder-secret-key-improved-v2',
    resave: true,
    ...
    try {
      ...
      try {
        ...
        const [user] = await db.insert(users)
          .values({
            username: req.body.username,
            email: req.body.username, // Use username as email since they're the same in our UI
            password: hashedPassword,
            role: 'user',
            language: 'ru',
            createdAt: new Date(),
