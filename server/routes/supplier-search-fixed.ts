...
async function callPythonParser(query: string, elements: number, userId: string, regions: any[] = ["ru"], sources: any = {}): Promise<SearchResult[]> {
  return new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      reject(new Error(`Failed to communicate with Python FastAPI microservice: ${errorMessage}`));
    }
    ...
    for (const result of results) {
        try {
            ...
            if (saveError.code === '23505') {
                ...
                console.error(`[SupplierSearch] Failed to save staging supplier ${result.domain}:`, saveError);
            }
        }
    }
