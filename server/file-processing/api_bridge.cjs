...
// Path to the Python scripts (relative to this file)
const ANALYZER_SCRIPT = path.join(__dirname, 'attachment_analyzer.py');
const SIMPLE_EXTRACTOR_SCRIPT = path.join(__dirname, 'simple_text_extractor.py');
...
/**
 * Process a single file using the simple extractor (fallback method)
 * 
 * @param {string} filePath - Path to the file to process
 * @param {string} mimeType - MIME type of the file
 * @returns {Promise<Object>} - The extraction results
 */
async function processSingleFile(filePath, mimeType) {
  return new Promise((resolve, reject) => {
    const pythonExecutable = 'python';
    
    if (!fs.existsSync(SIMPLE_EXTRACTOR_SCRIPT)) {
      return reject(new Error(`Simple extractor script not found at ${SIMPLE_EXTRACTOR_SCRIPT}`));
    }
    
    console.log(`Running simple extractor: ${pythonExecutable} ${SIMPLE_EXTRACTOR_SCRIPT} ${filePath} ${mimeType}`);
    
    const process = spawn(pythonExecutable, [
      SIMPLE_EXTRACTOR_SCRIPT,
      filePath,
      mimeType
    ]);
    ...
    process.stderr.on('data', (data) => {
      stderrData += data.toString();
      console.error(`Simple extractor stderr: ${data}`);
    });
    
    process.on('close', (code) => {
      if (code !== 0) {
        console.error(`Simple extractor exited with code ${code}`);
        console.error(`stderr: ${stderrData}`);
        return reject(new Error(`Simple extractor exited with code ${code}: ${stderrData}`));
      }
      ...
      try {
        ...
        console.error('Error parsing simple extractor output:', err);
        console.error('Raw output:', stdoutData);
        ...
        reject(new Error(`Failed to parse simple extractor output: ${err.message}`));
      }
    });
    
    process.on('error', (err) => {
      console.error('Error spawning simple extractor process:', err);
      reject(err);
    });
  });
}
