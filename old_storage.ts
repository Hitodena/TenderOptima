...
export class DatabaseStorage implements IStorage {
  ...
  async getSupplier(id: number): Promise<Supplier | undefined> {
    ...
    try {
      ...
      if (supplier) {
        ...
        // ╨а╨░╤Б╤И╨╕╤А╨╡╨╜╨╜╤Л╨╣ ╨┐╨╛╨╕╤Б╨║ - ╨┐╨╛╨┐╤А╨╛╨▒╤Г╨╡╨╝ ╨╜╨░╨╣╤В╨╕ ╨┐╨╛ string ID ╨╡╤Б╨╗╨╕ ╤З╨╕╤Б╨╗╨╛╨▓╨╛╨╣ ╨┐╨╛╨╕╤Б╨║ ╨╜╨╡ ╤Г╨┤╨░╨╗╤Б╤П
  ...
  async getSupplierByStringId(id: string): Promise<Supplier | undefined> {
    ...
    try {
      ...
      // ╨Ф╨╛╨┐╨╛╨╗╨╜╨╕╤В╨╡╨╗╤М╨╜╨░╤П ╨┐╨╛╨┐╤Л╤В╨║╨░ ╨┐╨╛╨╕╤Б╨║╨░ - ╨┐╤А╨╛╨▓╨╡╤А╨╕╨╝ ╨▓ ╤В╨░╨▒╨╗╨╕╤Ж╨╡ requestSuppliers
      ...
      try {
        ...
        if (supplierResponse) {
          ...
          // ╨Ш╤Б╨┐╨╛╨╗╤М╨╖╤Г╨╡╨╝ email ╨╕╨╖ supplierResponse
  ...
  async createSupplierResponse(response: InsertSupplierResponse & { messageId?: string }): Promise<SupplierResponse> {
    ...
    // ╨Ш╤Б╨┐╨╛╨╗╤М╨╖╤Г╨╡╨╝ ╤А╨░╤Б╤И╨╕╤А╨╡╨╜╨╜╤Л╨╣ ╤В╨╕╨┐ ╤Б ╨╛╨┐╤Ж╨╕╨╛╨╜╨░╨╗╤М╨╜╤Л╨╝ ╨┐╨╛╨╗╨╡╨╝ messageId
    const [supplierResponse] = await db
      .insert(supplierResponses)
      .values(response as any) // ╨Ш╤Б╨┐╨╛╨╗╤М╨╖╤Г╨╡╨╝ any, ╤З╤В╨╛╨▒╤Л ╨╛╨▒╨╛╨╣╤В╨╕ ╤В╨╕╨┐╨╕╨╖╨░╤Ж╨╕╤О ╨┤╨╛ ╨╛╨▒╨╜╨╛╨▓╨╗╨╡╨╜╨╕╤П ╤Б╤Е╨╡╨╝╤Л
      .returning();
  ...
  async getSupplierResponses(requestId: number | null, userId?: number): Promise<SupplierResponse[]> {
    ...
    let query = db.select({
      ...
      // ╨Э╨Х ╨▓╨║╨╗╤О╤З╨░╨╡╨╝ attachments - ╨╖╨░╨│╤А╤Г╨╢╨░╨╡╨╝ ╨╛╤В╨┤╨╡╨╗╤М╨╜╨╛ ╨┤╨╗╤П ╨▒╤Л╤Б╤В╤А╨╛╨╣ ╨╖╨░╨│╤А╤Г╨╖╨║╨╕
    }).from(supplierResponses);
    ...
    // Add empty attachments array - ╨▒╤Г╨┤╨╡╤В ╨╖╨░╨│╤А╤Г╨╢╨░╤В╤М╤Б╤П ╨╛╤В╨┤╨╡╨╗╤М╨╜╨╛
    const processedResults = results.map(response => ({
      ...response,
      attachments: [] // ╨Я╤Г╤Б╤В╨╛╨╣ ╨╝╨░╤Б╤Б╨╕╨▓ - ╨╖╨░╨│╤А╤Г╨╢╨░╨╡╨╝ ╨╛╤В╨┤╨╡╨╗╤М╨╜╨╛
    }));
  ...
  // ╨Я╨╛╨╗╤Г╤З╨╡╨╜╨╕╨╡ ╨╛╤В╨▓╨╡╤В╨╛╨▓ ╨┐╨╛╤Б╤В╨░╨▓╤Й╨╕╨║╨╛╨▓ ╨┤╨╗╤П ╨╜╨╡╤Б╨║╨╛╨╗╤М╨║╨╕╤Е ╨╖╨░╨┐╤А╨╛╤Б╨╛╨▓ ╤Б╤А╨░╨╖╤Г
  ...
  async toggleFavoriteResponse(id: number, userId?: number): Promise<SupplierResponse | undefined> {
    try {
      ...
      if (!currentResponse) {
        console.error(`╨Э╨╡ ╨╜╨░╨╣╨┤╨╡╨╜ ╨╛╤В╨▓╨╡╤В ╤Б ID ${id}${userId ? ` ╨┤╨╗╤П ╨┐╨╛╨╗╤М╨╖╨╛╨▓╨░╤В╨╡╨╗╤П ${userId}` : ''} ╨┤╨╗╤П ╨╕╨╖╨╝╨╡╨╜╨╡╨╜╨╕╤П ╤Б╤В╨░╤В╤Г╤Б╨░ ╨╕╨╖╨▒╤А╨░╨╜╨╜╨╛╨│╨╛`);
        return undefined;
      }
      
      // Toggle isFavorite state
      const newFavoriteState = !currentResponse.isFavorite;
      console.log(`╨Я╨╡╤А╨╡╨║╨╗╤О╤З╨╡╨╜╨╕╨╡ ╤Б╤В╨░╤В╤Г╤Б╨░ ╨╕╨╖╨▒╤А╨░╨╜╨╜╨╛╨│╨╛ ╨┤╨╗╤П ╨╛╤В╨▓╨╡╤В╨░ ${id}: ${currentResponse.isFavorite} -> ${newFavoriteState}`);
      ...
      console.error(`╨Ю╤И╨╕╨▒╨║╨░ ╨┐╤А╨╕ ╨╕╨╖╨╝╨╡╨╜╨╡╨╜╨╕╨╕ ╤Б╤В╨░╤В╤Г╤Б╨░ ╨╕╨╖╨▒╤А╨░╨╜╨╜╨╛╨│╨╛ ╤Г ╨╛╤В╨▓╨╡╤В╨░ ${id}:`, error);
      return undefined;
    }
  }
  ...
  async createContactItem(item: InsertContactItem): Promise<ContactItem> {
    try {
      // ╨Я╤А╨╛╨▓╨╡╤А╤П╨╡╨╝, ╤Б╤Г╤Й╨╡╤Б╤В╨▓╤Г╨╡╤В ╨╗╨╕ ╤Г╨╢╨╡ ╨║╨╛╨╜╤В╨░╨║╤В ╤Б ╤В╨░╨║╨╕╨╝ email ╨▓ ╤Н╤В╨╛╨╣ ╨│╤А╤Г╨┐╨┐╨╡
      // ╨Ф╨╛╨▒╨░╨▓╨╗╤П╨╡╨╝ ╨┐╤А╨╛╨▓╨╡╤А╨║╤Г userId ╨┤╨╗╤П ╨┐╤А╨░╨▓╨╕╨╗╤М╨╜╨╛╨╣ ╨╕╨╖╨╛╨╗╤П╤Ж╨╕╨╕ ╨┤╨░╨╜╨╜╤Л╤Е
      let query = and(
        eq(contactItems.groupId, item.groupId),
        eq(contactItems.email, item.email)
      );
      
      // ╨Х╤Б╨╗╨╕ ╨┐╨╡╤А╨╡╨┤╨░╨╜ userId, ╨┤╨╛╨▒╨░╨▓╨╗╤П╨╡╨╝ ╨╡╨│╨╛ ╨▓ ╤Г╤Б╨╗╨╛╨▓╨╕╨╡ ╨┤╨╗╤П ╨╕╨╖╨╛╨╗╤П╤Ж╨╕╨╕ ╨┤╨░╨╜╨╜╤Л╤Е
      if (item.userId) {
      ...
      // ╨Х╤Б╨╗╨╕ ╨║╨╛╨╜╤В╨░╨║╤В ╤Б ╤В╨░╨║╨╕╨╝ email ╤Г╨╢╨╡ ╤Б╤Г╤Й╨╡╤Б╤В╨▓╤Г╨╡╤В ╨▓ ╨│╤А╤Г╨┐╨┐╨╡, ╨▓╨╛╨╖╨▓╤А╨░╤Й╨░╨╡╨╝ ╨╡╨│╨╛
      if (existingContacts && existingContacts.length > 0) {
        console.log(`╨Ъ╨╛╨╜╤В╨░╨║╤В ╤Б email ${item.email} ╤Г╨╢╨╡ ╤Б╤Г╤Й╨╡╤Б╤В╨▓╤Г╨╡╤В ╨▓ ╨│╤А╤Г╨┐╨┐╨╡ ${item.groupId} ╨┤╨╗╤П ╨┐╨╛╨╗╤М╨╖╨╛╨▓╨░╤В╨╡╨╗╤П ${item.userId || '╨╗╤О╨▒╨╛╨│╨╛'}, ╨┐╤А╨╛╨┐╤Г╤Б╨║╨░╨╡╨╝ ╨┤╤Г╨▒╨╗╨╕╤А╨╛╨▓╨░╨╜╨╕╨╡`);
      ...
      console.log(`╨б╨╛╨╖╨┤╨░╨╜ ╨╜╨╛╨▓╤Л╨╣ ╨║╨╛╨╜╤В╨░╨║╤В: ${newItem.email} ╨▓ ╨│╤А╤Г╨┐╨┐╨╡ ${newItem.groupId} ╨┤╨╗╤П ╨┐╨╛╨╗╤М╨╖╨╛╨▓╨░╤В╨╡╨╗╤П ${newItem.userId || '╨╜╨╡ ╤Г╨║╨░╨╖╨░╨╜'}`);
      return newItem;
    } catch (error) {
      console.error(`╨Ю╤И╨╕╨▒╨║╨░ ╨┐╤А╨╕ ╤Б╨╛╨╖╨┤╨░╨╜╨╕╨╕ ╨║╨╛╨╜╤В╨░╨║╤В╨░ ${item.email} ╨▓ ╨│╤А╤Г╨┐╨┐╨╡ ${item.groupId}:`, error);
      // ╨Т ╤Б╨╗╤Г╤З╨░╨╡ ╨╛╤И╨╕╨▒╨║╨╕ ╨▓╤Б╨╡ ╤А╨░╨▓╨╜╨╛ ╨┐╤Л╤В╨░╨╡╨╝╤Б╤П ╨▓╤Б╤В╨░╨▓╨╕╤В╤М, ╨╡╤Б╨╗╨╕ ╤Н╤В╨╛ ╨╜╨╡ ╨┤╤Г╨▒╨╗╨╕╨║╨░╤В
      const [newItem] = await db
        .insert(contactItems)
        .values({
          ...item,
  ...
  async getContactItemsByGroupId(groupId: number, userId?: number): Promise<ContactItem[]> {
    try {
      console.log(`╨Ч╨░╨┐╤А╨╛╤Б ╨║╨╛╨╜╤В╨░╨║╤В╨╛╨▓ ╨┤╨╗╤П ╨│╤А╤Г╨┐╨┐╤Л ${groupId}${userId ? ` ╨╕ ╨┐╨╛╨╗╤М╨╖╨╛╨▓╨░╤В╨╡╨╗╤П ${userId}` : ''}`);
      
      // ╨б╤В╤А╨╛╨╕╨╝ ╨▒╨░╨╖╨╛╨▓╨╛╨╡ ╤Г╤Б╨╗╨╛╨▓╨╕╨╡ - ╨╛╨▒╤П╨╖╨░╤В╨╡╨╗╤М╨╜╨╛ ╨│╤А╤Г╨┐╨┐╨░ ╨┤╨╛╨╗╨╢╨╜╨░ ╤Б╨╛╨▓╨┐╨░╨┤╨░╤В╤М
      let conditions = eq(contactItems.groupId, groupId);
      
      // ╨Х╤Б╨╗╨╕ ╨┐╨╡╤А╨╡╨┤╨░╨╜ userId, ╨┤╨╛╨▒╨░╨▓╨╗╤П╨╡╨╝ ╨╡╨│╨╛ ╨▓ ╤Г╤Б╨╗╨╛╨▓╨╕╨╡ ╨┤╨╗╤П ╨╕╨╖╨╛╨╗╤П╤Ж╨╕╨╕ ╨┤╨░╨╜╨╜╤Л╤Е, ╨▓╨║╨╗╤О╤З╨░╤П legacy ╨║╨╛╨╜╤В╨░╨║╤В╤Л
      if (userId) {
        // ╨Я╨╛╨┤╨┤╨╡╤А╨╢╨╕╨▓╨░╨╡╨╝ legacy ╨║╨╛╨╜╤В╨░╨║╤В╤Л (╤Б null userId) ╨╕ ╨║╨╛╨╜╤В╨░╨║╤В╤Л ╤В╨╡╨║╤Г╤Й╨╡╨│╨╛ ╨┐╨╛╨╗╤М╨╖╨╛╨▓╨░╤В╨╡╨╗╤П
        // ╨н╤В╨╛ ╨▓╨░╨╢╨╜╨╛ ╨┤╨╗╤П ╨╛╨▒╤А╨░╤В╨╜╨╛╨╣ ╤Б╨╛╨▓╨╝╨╡╤Б╤В╨╕╨╝╨╛╤Б╤В╨╕ ╨╕ ╨║╨╛╤А╤А╨╡╨║╤В╨╜╨╛╨╣ ╤А╨░╨▒╨╛╤В╤Л ╤Б ╤Б╤Г╤Й╨╡╤Б╤В╨▓╤Г╤О╤Й╨╕╨╝╨╕ ╨┤╨░╨╜╨╜╤Л╨╝╨╕
        conditions = and(
          conditions, 
          or(
        ...
        console.log(`╨Я╤А╨╕╨╝╨╡╨╜╤П╨╡╨╝ ╤Д╨╕╨╗╤М╤В╤А╨░╤Ж╨╕╤О ╨┐╨╛ userId=${userId} ╨┤╨╗╤П ╨│╤А╤Г╨┐╨┐╤Л ${groupId} ╤Б ╨┐╨╛╨┤╨┤╨╡╤А╨╢╨║╨╛╨╣ legacy ╨║╨╛╨╜╤В╨░╨║╤В╨╛╨▓`);
      }
      ...
      console.log(`╨Э╨░╨╣╨┤╨╡╨╜╨╛ ${items.length} ╨║╨╛╨╜╤В╨░╨║╤В╨╛╨▓ ╨┤╨╗╤П ╨│╤А╤Г╨┐╨┐╤Л ${groupId} ╨┐╨╛╨╗╤М╨╖╨╛╨▓╨░╤В╨╡╨╗╤П ${userId || '╨╜╨╡ ╤Г╨║╨░╨╖╨░╨╜'}`);
      ...
      return []; // ╨Т ╤Б╨╗╤Г╤З╨░╨╡ ╨╛╤И╨╕╨▒╨║╨╕ ╨▓╨╛╨╖╨▓╤А╨░╤Й╨░╨╡╨╝ ╨┐╤Г╤Б╤В╨╛╨╣ ╨╝╨░╤Б╤Б╨╕╨▓ ╨┤╨╗╤П ╨▒╨╡╨╖╨╛╨┐╨░╤Б╨╜╨╛╤Б╤В╨╕
    }
  }

  // ╨Ю╨Я╨в╨Ш╨Ь╨Ш╨Ч╨Р╨ж╨Ш╨п: ╨Я╨╛╨╗╤Г╤З╨░╨╡╨╝ ╨║╨╛╨╗╨╕╤З╨╡╤Б╤В╨▓╨╛ ╨║╨╛╨╜╤В╨░╨║╤В╨╛╨▓ ╨┤╨╗╤П ╨▓╤Б╨╡╤Е ╨│╤А╤Г╨┐╨┐ ╨╛╨┤╨╜╨╕╨╝ ╨╖╨░╨┐╤А╨╛╤Б╨╛╨╝
  async getContactCountsByGroupIds(groupIds: number[], userId?: number): Promise<Record<number, number>> {
    try {
      console.log(`[OPTIMIZE] ╨Я╨╛╨╗╤Г╤З╨╡╨╜╨╕╨╡ ╨║╨╛╨╗╨╕╤З╨╡╤Б╤В╨▓╨░ ╨║╨╛╨╜╤В╨░╨║╤В╨╛╨▓ ╨┤╨╗╤П ${groupIds.length} ╨│╤А╤Г╨┐╨┐ ╨╛╨┤╨╜╨╕╨╝ ╨╖╨░╨┐╤А╨╛╤Б╨╛╨╝`);
      
      // ╨б╤В╤А╨╛╨╕╨╝ ╤Г╤Б╨╗╨╛╨▓╨╕╨╡ ╨┤╨╗╤П ╤Д╨╕╨╗╤М╤В╤А╨░╤Ж╨╕╨╕ ╨┐╨╛ ╨│╤А╤Г╨┐╨┐╨░╨╝
      ...
      // ╨Я╤А╨╡╨╛╨▒╤А╨░╨╖╤Г╨╡╨╝ ╤А╨╡╨╖╤Г╨╗╤М╤В╨░╤В ╨▓ ╨╛╨▒╤К╨╡╨║╤В ╨┤╨╗╤П ╨▒╤Л╤Б╤В╤А╨╛╨│╨╛ ╨┐╨╛╨╕╤Б╨║╨░
      const countMap: Record<number, number> = {};
      results.forEach(result => {
        countMap[result.groupId] = result.count;
      });

      console.log(`[OPTIMIZE] ╨Я╨╛╨╗╤Г╤З╨╡╨╜╨╛ ╨║╨╛╨╗╨╕╤З╨╡╤Б╤В╨▓╨╛ ╨║╨╛╨╜╤В╨░╨║╤В╨╛╨▓ ╨┤╨╗╤П ${results.length} ╨│╤А╤Г╨┐╨┐:`, countMap);
      
      return countMap;
    } catch (error) {
      console.error("╨Ю╤И╨╕╨▒╨║╨░ ╨┐╨╛╨╗╤Г╤З╨╡╨╜╨╕╤П ╨║╨╛╨╗╨╕╤З╨╡╤Б╤В╨▓╨░ ╨║╨╛╨╜╤В╨░╨║╤В╨╛╨▓ ╨┤╨╗╤П ╨│╤А╤Г╨┐╨┐:", error);
      throw error;
    }
  }
  ...
  async getContactItemsByEmail(groupId: number, email: string, userId?: number): Promise<ContactItem[]> {
    try {
      console.log(`╨Я╨╛╨╕╤Б╨║ ╨║╨╛╨╜╤В╨░╨║╤В╨░ ╤Б email ${email} ╨▓ ╨│╤А╤Г╨┐╨┐╨╡ ${groupId}${userId ? ` ╨┤╨╗╤П ╨┐╨╛╨╗╤М╨╖╨╛╨▓╨░╤В╨╡╨╗╤П ${userId}` : ''}`);
      ...
      // ╨Ф╨╛╨▒╨░╨▓╨╗╤П╨╡╨╝ ╤Д╨╕╨╗╤М╤В╤А╨░╤Ж╨╕╤О ╨┐╨╛ userId, ╨╡╤Б╨╗╨╕ ╨╛╨╜ ╤Г╨║╨░╨╖╨░╨╜ ╨┤╨╗╤П ╨╕╨╖╨╛╨╗╤П╤Ж╨╕╨╕ ╨┤╨░╨╜╨╜╤Л╤Е
      if (userId) {
        // ╨Ш╨б╨Я╨а╨Р╨Т╨Ы╨Х╨Э╨Ш╨Х: ╨Ш╤Б╨┐╨╛╨╗╤М╨╖╤Г╨╡╨╝ ╤В╤Г ╨╢╨╡ ╨╗╨╛╨│╨╕╨║╤Г ╤Д╨╕╨╗╤М╤В╤А╨░╤Ж╨╕╨╕ ╤З╤В╨╛ ╨╕ ╨▓ getContactItemsByGroupId
        // ╨Я╨╛╨┤╨┤╨╡╤А╨╢╨╕╨▓╨░╨╡╨╝ legacy ╨║╨╛╨╜╤В╨░╨║╤В╤Л (╤Б null userId) ╨╕ ╨║╨╛╨╜╤В╨░╨║╤В╤Л ╤В╨╡╨║╤Г╤Й╨╡╨│╨╛ ╨┐╨╛╨╗╤М╨╖╨╛╨▓╨░╤В╨╡╨╗╤П
        conditions = and(
          conditions,
          or(
        ...
        console.log(`╨Я╤А╨╕╨╝╨╡╨╜╤П╨╡╨╝ ╨│╨╕╨▒╨║╤Г╤О ╤Д╨╕╨╗╤М╤В╤А╨░╤Ж╨╕╤О ╨┐╨╛ userId=${userId} ╨┤╨╗╤П email=${email} ╨▓ ╨│╤А╤Г╨┐╨┐╨╡ ${groupId} ╤Б ╨┐╨╛╨┤╨┤╨╡╤А╨╢╨║╨╛╨╣ legacy ╨║╨╛╨╜╤В╨░╨║╤В╨╛╨▓`);
      }
      ...
      console.log(`╨Э╨░╨╣╨┤╨╡╨╜╨╛ ${items.length} ╨║╨╛╨╜╤В╨░╨║╤В╨╛╨▓ ╤Б email ${email} ╨▓ ╨│╤А╤Г╨┐╨┐╨╡ ${groupId} ╨┐╨╛╨╗╤М╨╖╨╛╨▓╨░╤В╨╡╨╗╤П ${userId || '╨╜╨╡ ╤Г╨║╨░╨╖╨░╨╜'}`);
      
      // ╨Т╨╛╨╖╨▓╤А╨░╤Й╨░╨╡╨╝ ╨║╨╛╨╜╤В╨░╨║╤В╤Л ╨▒╨╡╨╖ ╨╗╨╕╤И╨╜╨╕╤Е ╨┐╨╛╨╗╨╡╨╣
      return items;
    } catch (error) {
      console.error(`╨Ю╤И╨╕╨▒╨║╨░ ╨┐╤А╨╕ ╨┐╨╛╨╕╤Б╨║╨╡ ╨║╨╛╨╜╤В╨░╨║╤В╨░ ╤Б email ${email} ╨▓ ╨│╤А╤Г╨┐╨┐╨╡ ${groupId}${userId ? ` ╨┤╨╗╤П ╨┐╨╛╨╗╤М╨╖╨╛╨▓╨░╤В╨╡╨╗╤П ${userId}` : ''}:`, error);
      return [];
    }
  }
  ...
  async getContactById(contactId: number): Promise<ContactItem | null> {
    try {
      console.log(`╨Я╨╛╨╗╤Г╤З╨╡╨╜╨╕╨╡ ╨║╨╛╨╜╤В╨░╨║╤В╨░ ╤Б ID ${contactId}`);
      ...
      if (!contact) {
        console.log(`╨Ъ╨╛╨╜╤В╨░╨║╤В ╤Б ID ${contactId} ╨╜╨╡ ╨╜╨░╨╣╨┤╨╡╨╜`);
        return null;
      }
  ...
  async updateContactItem(id: number, item: Partial<InsertContactItem>): Promise<ContactItem | undefined> {
    try {
      console.log(`╨Ю╨▒╨╜╨╛╨▓╨╗╨╡╨╜╨╕╨╡ ╨║╨╛╨╜╤В╨░╨║╤В╨░ ╤Б ID ${id}:`, item);
      ...
      console.log(`╨Ъ╨╛╨╜╤В╨░╨║╤В ╤Б ID ${id} ╤Г╤Б╨┐╨╡╤И╨╜╨╛ ╨╛╨▒╨╜╨╛╨▓╨╗╨╡╨╜:`, updatedItem);
      return updatedItem;
    } catch (error) {
      console.error(`╨Ю╤И╨╕╨▒╨║╨░ ╨╛╨▒╨╜╨╛╨▓╨╗╨╡╨╜╨╕╤П ╨║╨╛╨╜╤В╨░╨║╤В╨░ ╤Б ID ${id}:`, error);
      return undefined;
    }
  }
  ...
  async addContactsToGroup(groupId: number, contacts: Array<Partial<InsertContactItem>>, userId?: number): Promise<ContactItem[]> {
    ...
    try {
      // ╨Я╤А╨╛╨▓╨╡╤А╤П╨╡╨╝ ╤Б╤Г╤Й╨╡╤Б╤В╨▓╨╛╨▓╨░╨╜╨╕╨╡ ╨│╤А╤Г╨┐╨┐╤Л ╤Б ╨┐╤А╨╛╨▓╨╡╤А╨║╨╛╨╣ userId ╨┤╨╗╤П ╨╕╨╖╨╛╨╗╤П╤Ж╨╕╨╕ ╨┤╨░╨╜╨╜╤Л╤Е
      ...
      // ╨Ш╤Б╨┐╨╛╨╗╤М╨╖╤Г╨╡╨╝ userId ╨╕╨╖ ╨│╤А╤Г╨┐╨┐╤Л, ╨╡╤Б╨╗╨╕ ╨╛╨╜ ╨╜╨╡ ╤Г╨║╨░╨╖╨░╨╜ ╤П╨▓╨╜╨╛
      const actualUserId = userId || group.userId;
      
      console.log(`Using userId=${actualUserId} for adding contacts to group ${groupId}`);
      
      // ╨Я╨╛╨┤╨│╨╛╤В╨░╨▓╨╗╨╕╨▓╨░╨╡╨╝ ╨║╨╛╨╜╤В╨░╨║╤В╤Л ╨║ ╨▓╤Б╤В╨░╨▓╨║╨╡,
      // ╤Г╨▒╨╡╨╢╨┤╨░╨╡╨╝╤Б╤П, ╤З╤В╨╛ ╤Г ╨║╨░╨╢╨┤╨╛╨│╨╛ ╨║╨╛╨╜╤В╨░╨║╤В╨░ ╨╡╤Б╤В╤М ╨▓╤Б╨╡ ╨╜╨╡╨╛╨▒╤Е╨╛╨┤╨╕╨╝╤Л╨╡ ╨┐╨╛╨╗╤П
      // ╨▓╨║╨╗╤О╤З╨░╤П userId ╨┤╨╗╤П ╨╕╨╖╨╛╨╗╤П╤Ж╨╕╨╕ ╨┤╨░╨╜╨╜╤Л╤Е ╨╝╨╡╨╢╨┤╤Г ╨░╨║╨║╨░╤Г╨╜╤В╨░╨╝╨╕
      const contactsToInsert = contacts.map(contact => ({
        groupId: groupId,
        userId: actualUserId, // ╨Т╤Б╨╡╨│╨┤╨░ ╨╕╤Б╨┐╨╛╨╗╤М╨╖╤Г╨╡╨╝ userId ╨┤╨╗╤П ╨┐╤А╨░╨▓╨╕╨╗╤М╨╜╨╛╨╣ ╨╕╨╖╨╛╨╗╤П╤Ж╨╕╨╕ ╨┤╨░╨╜╨╜╤Л╤Е
        email: contact.email || '',
  ...
  async addSupplierToContactGroup(data: InsertRequestSupplierGroup): Promise<RequestSupplierGroup> {
    try {
      // ╨Я╤А╨╛╨▓╨╡╤А╤П╨╡╨╝, ╤Б╤Г╤Й╨╡╤Б╤В╨▓╤Г╨╡╤В ╨╗╨╕ ╤Г╨╢╨╡ ╨┐╨╛╤Б╤В╨░╨▓╤Й╨╕╨║ ╨▓ ╤Н╤В╨╛╨╣ ╨│╤А╤Г╨┐╨┐╨╡
      ...
      // ╨Х╤Б╨╗╨╕ ╤Б╨▓╤П╨╖╤М ╤Г╨╢╨╡ ╤Б╤Г╤Й╨╡╤Б╤В╨▓╤Г╨╡╤В, ╨▓╨╛╨╖╨▓╤А╨░╤Й╨░╨╡╨╝ ╨╡╤С
      if (existingRelations && existingRelations.length > 0) {
        console.log(`╨Я╨╛╤Б╤В╨░╨▓╤Й╨╕╨║ ${data.requestSupplierId} ╤Г╨╢╨╡ ╤Б╤Г╤Й╨╡╤Б╤В╨▓╤Г╨╡╤В ╨▓ ╨│╤А╤Г╨┐╨┐╨╡ ${data.contactGroupId}, ╨┐╤А╨╛╨┐╤Г╤Б╨║╨░╨╡╨╝ ╨┤╤Г╨▒╨╗╨╕╤А╨╛╨▓╨░╨╜╨╕╨╡`);
      ...
      console.log(`╨Я╨╛╤Б╤В╨░╨▓╤Й╨╕╨║ ${data.requestSupplierId} ╨┤╨╛╨▒╨░╨▓╨╗╨╡╨╜ ╨▓ ╨│╤А╤Г╨┐╨┐╤Г ╨║╨╛╨╜╤В╨░╨║╤В╨╛╨▓ ${data.contactGroupId}`);
      return newRelation;
    } catch (error) {
  ...
  async updateSupplierGroups(supplierId: number, groupIds: number[]): Promise<{ success: boolean }> {
    try {
      // ╨Я╤А╨╛╨▓╨╡╤А╨╕╨╝, ╨╡╤Б╨╗╨╕ ╤Н╤В╨╛ API-╨┐╨╛╤Б╤В╨░╨▓╤Й╨╕╨║ (╤Б ╨╛╤В╤А╨╕╤Ж╨░╤В╨╡╨╗╤М╨╜╤Л╨╝ ID)
      const isApiSupplier = supplierId < 0;
      
      let requestSupplierId: number;
      
      if (isApiSupplier) {
        // ╨Ф╨╗╤П API-╨┐╨╛╤Б╤В╨░╨▓╤Й╨╕╨║╨╛╨▓ ╤В╤А╨╡╨▒╤Г╨╡╤В╤Б╤П ╤Б╨╜╨░╤З╨░╨╗╨░ ╤Б╨╛╨╖╨┤╨░╤В╤М ╨╖╨░╨┐╨╕╤Б╤М ╨▓ ╤В╨░╨▒╨╗╨╕╤Ж╨╡ requestSuppliers
        // ╨Я╤А╨╛╨▓╨╡╤А╨╕╨╝, ╨╡╤Б╤В╤М ╨╗╨╕ ╤Г╨╢╨╡ ╤В╨░╨║╨░╤П ╨╖╨░╨┐╨╕╤Б╤М
        ...
        if (existingRecords.length > 0) {
          // ╨Ш╤Б╨┐╨╛╨╗╤М╨╖╤Г╨╡╨╝ ╤Б╤Г╤Й╨╡╤Б╤В╨▓╤Г╤О╤Й╤Г╤О ╨╖╨░╨┐╨╕╤Б╤М
          requestSupplierId = existingRecords[0].id;
        } else {
          // ╨Э╨░╨╣╨┤╨╡╨╝ ╤Б╤Г╤Й╨╡╤Б╤В╨▓╤Г╤О╤Й╨╕╨╣ ╨╖╨░╨┐╤А╨╛╤Б ╨┤╨╗╤П ╨╕╤Б╨┐╨╛╨╗╤М╨╖╨╛╨▓╨░╨╜╨╕╤П ╨▓ ╨║╨░╤З╨╡╤Б╤В╨▓╨╡ ╨┐╤А╨╕╨▓╤П╨╖╨║╨╕
          // ╨Э╨░╨╝ ╨╜╤Г╨╢╨╡╨╜ ╨▓╨░╨╗╨╕╨┤╨╜╤Л╨╣ ID ╨╖╨░╨┐╤А╨╛╤Б╨░, ╤В.╨║. ╨▓ ╨▒╨░╨╖╨╡ ╨╡╤Б╤В╤М ╨╛╨│╤А╨░╨╜╨╕╤З╨╡╨╜╨╕╨╡ ╨▓╨╜╨╡╤И╨╜╨╡╨│╨╛ ╨║╨╗╤О╤З╨░
          ...
          if (existingRequests.length === 0) {
            throw new Error("╨Э╨╡ ╨╜╨░╨╣╨┤╨╡╨╜╨╛ ╨╜╨╕ ╨╛╨┤╨╜╨╛╨│╨╛ ╨╖╨░╨┐╤А╨╛╤Б╨░ ╨┤╨╗╤П ╨┐╤А╨╕╨▓╤П╨╖╨║╨╕ ╨┐╨╛╤Б╤В╨░╨▓╤Й╨╕╨║╨░. ╨б╨╛╨╖╨┤╨░╨╣╤В╨╡ ╤Е╨╛╤В╤П ╨▒╤Л ╨╛╨┤╨╕╨╜ ╨╖╨░╨┐╤А╨╛╤Б.");
          }
          ...
          try {
            console.log(`╨б╨╛╨╖╨┤╨░╨╜╨╕╨╡ ╨▓╤А╨╡╨╝╨╡╨╜╨╜╨╛╨╣ ╨╖╨░╨┐╨╕╤Б╨╕ requestSupplier ╨┤╨╗╤П API-╨┐╨╛╤Б╤В╨░╨▓╤Й╨╕╨║╨░ ${supplierId} ╤Б requestId=${validRequestId}`);
            
            const [createdRecord] = await db
              .insert(requestSuppliers)
              .values({
                requestId: validRequestId, // ╨Ш╤Б╨┐╨╛╨╗╤М╨╖╤Г╨╡╨╝ ╤Б╤Г╤Й╨╡╤Б╤В╨▓╤Г╤О╤Й╨╕╨╣ ╨╖╨░╨┐╤А╨╛╤Б ╨▓╨╝╨╡╤Б╤В╨╛ ╤Д╨╡╨╣╨║╨╛╨▓╨╛╨│╨╛ ID
            ...
            console.log(`╨б╨╛╨╖╨┤╨░╨╜╨░ ╨╖╨░╨┐╨╕╤Б╤М requestSupplier ╤Б ID=${requestSupplierId} ╨┤╨╗╤П API-╨┐╨╛╤Б╤В╨░╨▓╤Й╨╕╨║╨░ ${supplierId}`);
          } catch (dbError) {
            console.error(`╨Ю╤И╨╕╨▒╨║╨░ ╨┐╤А╨╕ ╤Б╨╛╨╖╨┤╨░╨╜╨╕╨╕ ╨╖╨░╨┐╨╕╤Б╨╕ requestSupplier ╨┤╨╗╤П API-╨┐╨╛╤Б╤В╨░╨▓╤Й╨╕╨║╨░ ${supplierId}:`, dbError);
            throw dbError;
          }
        }
      } else {
        // ╨Ю╨▒╤А╨░╨▒╨╛╤В╨║╨░ ╨┐╨╛╤Б╤В╨░╨▓╤Й╨╕╨║╨╛╨▓ ╨╕╨╖ ╨▒╨░╨╖╤Л ╨┤╨░╨╜╨╜╤Л╤Е (╨┐╨╛╨╗╨╛╨╢╨╕╤В╨╡╨╗╤М╨╜╤Л╨╣ ID)
        const supplier = await this.getSupplier(supplierId);
        
        if (supplier) {
          // ╨Я╤А╨╛╨▓╨╡╤А╤П╨╡╨╝, ╨╡╤Б╤В╤М ╨╗╨╕ ╨╖╨░╨┐╨╕╤Б╨╕ requestSupplier ╨┤╨╗╤П ╤Н╤В╨╛╨│╨╛ ╨┐╨╛╤Б╤В╨░╨▓╤Й╨╕╨║╨░
          const supplierRecords = await db
            .select()
            .from(requestSuppliers)
            .where(eq(requestSuppliers.supplierId, supplierId.toString()));
          ...
          if (supplierRecords.length > 0) {
            // ╨Ш╤Б╨┐╨╛╨╗╤М╨╖╤Г╨╡╨╝ ╨┐╨╡╤А╨▓╤Г╤О ╨╖╨░╨┐╨╕╤Б╤М requestSupplier ╨┤╨╗╤П ╨╛╨▒╨╜╨╛╨▓╨╗╨╡╨╜╨╕╤П ╨│╤А╤Г╨┐╨┐
            requestSupplierId = supplierRecords[0].id;
          } else {
            // ╨Э╨╡╤В ╨╖╨░╨┐╨╕╤Б╨╕ requestSupplier ╨┤╨╗╤П ╤Н╤В╨╛╨│╨╛ ╨┐╨╛╤Б╤В╨░╨▓╤Й╨╕╨║╨░
            // ╨б╨╛╨╖╨┤╨░╨┤╨╕╨╝ ╨▓╤А╨╡╨╝╨╡╨╜╨╜╤Г╤О ╨╖╨░╨┐╨╕╤Б╤М, ╨╕╤Б╨┐╨╛╨╗╤М╨╖╤Г╤П ╤Б╤Г╤Й╨╡╤Б╤В╨▓╤Г╤О╤Й╨╕╨╣ ╨╖╨░╨┐╤А╨╛╤Б
            // ╨Э╨░╨╣╨┤╨╡╨╝ ╤Б╤Г╤Й╨╡╤Б╤В╨▓╤Г╤О╤Й╨╕╨╣ ╨╖╨░╨┐╤А╨╛╤Б ╨┤╨╗╤П ╨╕╤Б╨┐╨╛╨╗╤М╨╖╨╛╨▓╨░╨╜╨╕╤П ╨▓ ╨║╨░╤З╨╡╤Б╤В╨▓╨╡ ╨┐╤А╨╕╨▓╤П╨╖╨║╨╕
            const existingRequests = await db
              .select()
              .from(searchRequests)
            ...
            console.log(`╨б╨╛╨╖╨┤╨░╨╜╨╕╨╡ ╨▓╤А╨╡╨╝╨╡╨╜╨╜╨╛╨╣ ╨╖╨░╨┐╨╕╤Б╨╕ requestSupplier ╨┤╨╗╤П ╨┐╨╛╤Б╤В╨░╨▓╤Й╨╕╨║╨░ ${supplierId} ╤Б requestId=${validRequestId}`);
            ...
            try {
              ...
              console.log(`╨б╨╛╨╖╨┤╨░╨╜╨░ ╨╖╨░╨┐╨╕╤Б╤М requestSupplier ╤Б ID=${createdRecordId} ╨┤╨╗╤П ╨┐╨╛╤Б╤В╨░╨▓╤Й╨╕╨║╨░ ${supplierId}`);
            } catch (dbError) {
              console.error(`╨Ю╤И╨╕╨▒╨║╨░ ╨┐╤А╨╕ ╤Б╨╛╨╖╨┤╨░╨╜╨╕╨╕ ╨╖╨░╨┐╨╕╤Б╨╕ requestSupplier ╨┤╨╗╤П ╨┐╨╛╤Б╤В╨░╨▓╤Й╨╕╨║╨░ ${supplierId}:`, dbError);
              throw dbError;
            }
      ...
      // ╨У╤А╤Г╨┐╨┐╤Л ╨┤╨╗╤П ╨┤╨╛╨▒╨░╨▓╨╗╨╡╨╜╨╕╤П (╨▓ groupIds, ╨╜╨╛ ╨╜╨╡ ╨▓ currentGroupIds)
      const groupsToAdd = groupIds.filter(id => !currentGroupIds.includes(id));
      
      // ╨У╤А╤Г╨┐╨┐╤Л ╨┤╨╗╤П ╤Г╨┤╨░╨╗╨╡╨╜╨╕╤П (╨▓ currentGroupIds, ╨╜╨╛ ╨╜╨╡ ╨▓ groupIds)
  ...
  async getSupplierMessages(requestSupplierId: number, userId?: number): Promise<SupplierMessage[]> {
    try {
      ...
      // ╨Я╤А╨╛╨▓╨╡╤А╤П╨╡╨╝, ╨╡╤Б╤В╤М ╨╗╨╕ ╤В╨░╨║╨╛╨╣ requestSupplier ╨╕ ╨┐╤А╨╕╨╜╨░╨┤╨╗╨╡╨╢╨╕╤В ╨╗╨╕ ╨╛╨╜ ╤Г╨║╨░╨╖╨░╨╜╨╜╨╛╨╝╤Г ╨┐╨╛╨╗╤М╨╖╨╛╨▓╨░╤В╨╡╨╗╤О
      ...
      // ╨Х╤Б╨╗╨╕ ╨┐╨╡╤А╨╡╨┤╨░╨╜ userId, ╨┤╨╛╨▒╨░╨▓╨╗╤П╨╡╨╝ ╨┐╤А╨╛╨▓╨╡╤А╨║╤Г ╨╜╨░ ╨┐╤А╨╕╨╜╨░╨┤╨╗╨╡╨╢╨╜╨╛╤Б╤В╤М ╨┐╨╛╤Б╤В╨░╨▓╤Й╨╕╨║╨░ ╨┐╨╛╨╗╤М╨╖╨╛╨▓╨░╤В╨╡╨╗╤О
      if (userId) {
        // ╨Ф╨╗╤П ╨┐╤А╨╛╨▓╨╡╤А╨║╨╕ ╤Б╨▓╤П╨╖╨╕ ╤Б userId, ╨╜╨░╨╝ ╨╜╤Г╨╢╨╜╨╛ joined query ╤З╨╡╤А╨╡╨╖ searchRequests
        ...
        // ╨Х╤Б╨╗╨╕ userId ╨╜╨╡ ╨┐╨╡╤А╨╡╨┤╨░╨╜, ╨┐╤А╨╛╨▓╨╡╤А╤П╨╡╨╝ ╤В╨╛╨╗╤М╨║╨╛ ╤Б╤Г╤Й╨╡╤Б╤В╨▓╨╛╨▓╨░╨╜╨╕╨╡ requestSupplier
      ...
      // ╨Х╤Б╨╗╨╕ userId ╨┐╨╡╤А╨╡╨┤╨░╨╜, ╨┤╨╛╨▒╨░╨▓╨╗╤П╨╡╨╝ ╤Г╤Б╨╗╨╛╨▓╨╕╨╡ ╤Д╨╕╨╗╤М╤В╤А╨░╤Ж╨╕╨╕ ╨┐╨╛ userId
      // ╨Э╨╛ ╤В╨░╨║╨╢╨╡ ╨▓╨║╨╗╤О╤З╨░╨╡╨╝ ╤Б╨╛╨╛╨▒╤Й╨╡╨╜╨╕╤П ╤Б userId = null, ╤В╨░╨║ ╨║╨░╨║ ╨╛╨╜╨╕ ╨╝╨╛╨│╤Г╤В ╨▒╤Л╤В╤М ╤Б╨╕╤Б╤В╨╡╨╝╨╜╤Л╨╝╨╕
      let whereCondition;
      if (userId) {
        whereCondition = and(
          baseCondition,
  ...
  async addSupplierMessage(message: InsertSupplierMessage): Promise<SupplierMessage> {
    try {
      console.log(`[storage] Adding new message for requestSupplierId ${message.requestSupplierId}`, {
        ...
        fullContent: message.content // ╨Я╨╛╨╗╨╜╨╛╨╡ ╤Б╨╛╨┤╨╡╤А╨╢╨╕╨╝╨╛╨╡ ╨┤╨╗╤П ╨╛╤В╨╗╨░╨┤╨║╨╕
      });
      
      // ╨Я╤А╨╛╨▓╨╡╤А╤П╨╡╨╝ ╤Б╤Г╤Й╨╡╤Б╤В╨▓╨╛╨▓╨░╨╜╨╕╨╡ requestSupplierId
      const requestSupplier = await db
        .select()
        .from(requestSuppliers)
      ...
      // ╨б╨╛╨╖╨┤╨░╨╡╨╝ ╨╛╨▒╤К╨╡╨║╤В ╤Б ╨┤╨░╨╜╨╜╤Л╨╝╨╕, ╨╕╤Б╨║╨╗╤О╤З╨░╤П ╨┐╨╛╨╗╨╡ subject, ╨║╨╛╤В╨╛╤А╨╛╨│╨╛ ╨╜╨╡╤В ╨▓ ╨▒╨░╨╖╨╡ ╨┤╨░╨╜╨╜╤Л╤Е
      const messageData = {
      ...
      // ╨Ш╤Б╨┐╨╛╨╗╤М╨╖╤Г╨╡╨╝ ╨┐╤А╤П╨╝╤Г╤О ╤Б╤Е╨╡╨╝╤Г ╤Б ╨╕╤Б╨┐╤А╨░╨▓╨╗╨╡╨╜╨╜╤Л╨╝╨╕ ╨┐╨╛╨╗╤П╨╝╨╕
  ...
  async createUnprocessedEmail(email: InsertUnprocessedEmail): Promise<UnprocessedEmail | null> {
    try {
      // ╨Я╤А╨╛╨▓╨╡╤А╤П╨╡╨╝, ╤Б╤Г╤Й╨╡╤Б╤В╨▓╤Г╨╡╤В ╨╗╨╕ ╤Г╨╢╨╡ email ╤Б ╤В╨░╨║╨╕╨╝ message_id
      ...
      if (existing.length > 0) {
        ...
        return null; // ╨Т╨╛╨╖╨▓╤А╨░╤Й╨░╨╡╨╝ null ╨▓╨╝╨╡╤Б╤В╨╛ ╤Б╤Г╤Й╨╡╤Б╤В╨▓╤Г╤О╤Й╨╡╨╣ ╨╖╨░╨┐╨╕╤Б╨╕
      }
